<template>
  <div class="database">
    <div class="container">
      <h1 v-if="table" class="h1-dark">
        <a @click.prevent="$router.push('/database')" href="">База данных/</a>{{ table.name }}
      </h1>
      <TableControl
        v-if="table"
      ...
      <div v-if="!loading && table" class="database__tables">
        ...
        <div class="database__table" v-for="table in tables" :key="table._id">
          <div class="database__table__name">
            <router-link :to="`/card/${table._id}`">{{ table.name }}</router-link>
          </div>
      ...
      <div class="database__pagination small-text" v-if="pages > 1 && table" v-show="!loading">
